...
execute as @e[tag=move] store result entity @s Motion[0] double .0095 run scoreboard players get @e[tag=det,type=marker,limit=1,sort=nearest,tag=!done] xPos
execute as @e[tag=move] store result entity @s Motion[2] double .0095 run scoreboard players get @e[tag=det,type=marker,limit=1,sort=nearest,tag=!done] zPos
